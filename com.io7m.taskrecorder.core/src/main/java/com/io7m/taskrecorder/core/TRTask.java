...
public final class TRTask<T> implements TRStepType
{
  private final Logger logger;
  private final String name;
  private final LinkedList<TRStepType> steps;
  private final List<TRStepType> stepsRead;
    ...
    final String inName)
  {
    ...
    this.steps.add(new TRStep(this.logger, this.name));
  ...
  public static <T> TRTask<T> create(
    final Logger logger,
    final String name)
  {
    return new TRTask<T>(logger, UUID.randomUUID(), name);
  }
    ...
    final String newName)
  {
    ...
    this.steps.add(new TRStep(this.logger, newName));
  }
  ...
  public <U> TRTask<U> beginSubtask(
    final String newName)
  {
    this.logger.debug("begin subtask: {}", newName);
    final var subtask = new TRTask<U>(this.logger, this.id, newName);
  ...
  {
    return "[TrTask %s %s]".formatted(this.id, this.name);
  }
